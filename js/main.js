...
function fetchHumanAnswers(oQuestions){
    ...
    $.ajax({
        ...
        if(oData.length > 0){
            console.log("[OK] Fetched "+ oData.length +" answers for user id " + userId+":");
            console.log(oData);
            ...
            if($("#dogViewer").length > 0){
                showPopup("Welcome to Topdog!", "To start matching, go to 'My Profile' and fill out the form.");
            }

            if($("#boxQuestionsHuman").length > 0){
                showPopup("My Profile", "Answer these questions and return to 'Matches' to find your top dog!");
            }
...
function renderDogs(oDogs){
    ...
    $.each(oDogs,function(index){
        ...
        $("#dogViewer").append(`
            ...
            <div class="compFrame fadeIn dogCard" data-dog-id="${this.dog_id}">
                ...
                <div class="dogDetail">
                    ...
                    <div class="dogDescription">${truncateString(this.description)}</div>
                </div>
        ...
        if (typeof(dogId) != 'undefined' && this.matchScore > 0){ // if the dogId is not undefined, and has replies, run the following

            if (dogId == this.dog_id){      // only for the dog where there is a match between object and dogId
